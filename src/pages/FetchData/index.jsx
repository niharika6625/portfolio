...
const { apiFetch } = PROJECT_DESCRIPTION;
...
const FetchData = () => {
  ...
  const searchData = () => {
    const tempArr = fetchedDataArr.filter((obj) => obj[thirdKey].includes(searchInputValue));
    if (tempArr.length === 0) {
      setFetchedDataArr([]);
    } else {
      createTable(tempArr);
  ...
  const tableClicked = (event) => {
    if (event.target.dataset.type === 'delete') {
      const updatedArr = fetchedDataArr.filter((obj) => obj.id !== event.target.dataset.id);
